...
bool InitData()
{
    ...
    else
    {
        ...
        font_mark = TTF_OpenFont("font//dlxfont.ttf", 25);
        if(font_mark == NULL)
        {
            success = false;
        }
...
{    
    ...
    GeometricFormat rectangle_size_play(170, 425, 200, 40);
    ColorData color_data(201, 231, 255);
    Geometric ::RenderRectangle(rectangle_size_play, color_data, g_screen);
    
    GeometricFormat outLineSize(171, 426, 199, 38);
    ColorData color_data2(255, 251, 8);
    Geometric ::RenderOutline(outLineSize, color_data2, g_screen);

    GeometricFormat rectangle_size_exit(910, 425, 200, 40);
    ColorData color_data_exit(201, 231, 255);
    Geometric ::RenderRectangle(rectangle_size_exit, color_data_exit, g_screen);
    
    GeometricFormat outLineSizeExit(911, 426, 199, 38);
    ColorData color_exit(255, 251, 8);
    Geometric ::RenderOutline(outLineSizeExit, color_exit, g_screen);
...
int ShowGameOver(SDL_Event g_event, bool is_win)
{
    ...
    pos_mark.x = SCREEN_WIDTH * 0.32;
    pos_mark.y = SCREEN_HEIGHT * 0.1;

    TextObject text_menu[Item];
    TextObject text_mark;
    ...
    GeometricFormat rectangle1(170, 425, 200, 40);
    ColorData color_data1(201, 231, 255);
    Geometric ::RenderRectangle(rectangle1, color_data1, g_screen);
    
    GeometricFormat outLineSize1(171,426, 199, 38);
    ColorData color_data3(255, 251, 8);
    Geometric ::RenderOutline(outLineSize1, color_data3, g_screen);
    ...
    text_mark.SetText("YOUR MARK IS: ");
    text_mark.SetColor(TextObject ::YELLOW_TEXT);
    text_mark.LoadFromRenderText(font_mark, g_screen);
    text_mark.RenderText(g_screen, pos_mark.x, pos_mark.y);
    text_mark.SetRech(pos_mark.x, pos_mark.y);

    bool selected[Item] = {0, 0};
    ...
    while (true)
    {
        ...
        mark_game.RenderText(g_screen, SCREEN_WIDTH*0.61, SCREEN_HEIGHT*0.1);
...
int main(int argc, char *argv[])
{
    ...
    while (status != -1)
    {
        ...
        int xPosBoss = MAX_MAP_X * TILE_SIZE - (SCREEN_WIDTH * 0.3);
        bossObject.set_xpos(xPosBoss);
        bossObject.set_ypos(0);
        ...
        num_die = 0;
        player_power.Init(g_screen);
        p_player.setpos(0, 300);
        p_player.ReSetMoneyCount();
        mark_value = 0;
        money_count = 0;
        status = 1;
